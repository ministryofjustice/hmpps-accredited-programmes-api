...
| PR-13 | `PR-13-round-2-docs-and-handover.md` | `APG-2546/planning-sar-field-removals` (target — planning branch, NOT main) | _n/a (planning-branch commits, not a main PR)_ | ⬜ **13b ready to start; 13c pending Raby PDF drop; 13d comms drafted awaiting Raby send** — anchored to `99264496`, doc reshaped 2026-08-18 pm to 13b/13c/13d split | _pending_ | _pending_ | **Shape-corrected 2026-08-18 pm** after executing agent flagged that the earlier doc conflated a docs-only planning-branch close-out with a fresh `main` PR. Precedent = PR-6 round-1 handover (never merged to main). Split: **13b** — fresh agent, planning-branch commits, docs close-out + `handover/` scaffolding + comms drafts. **13c** — Raby (or Deborah's dev) generates Branston PDF via Cameron's SAR dev-service (Option 1, full-chrome), drops in at `doc/planning/APG-2546/handover/round-2-sample.pdf`. **13d** — Raby sends OSAR email + Deborah Slack DM in the planning-agent chat once 13c lands; DELIVERY-LOG updated with send timestamps after. R7 `.gitignore` sweep explicitly deferred (accepted paper-cut; details on ROUND-2-PLAN R7 row). |
