...
| PR-12 | `PR-12-round-2-hygiene-tidy.md` | `APG-2546/round-2-hygiene-tidy` | _pending_ | _pending_ | _pending_ | **Unblocked 2026-08-17 by PR-11 merge (`47488c8a`).** Pre-verification greps against `47488c8a` in 2026-08-17 (late evening) timeline entry — most sweeps CONFIRMED CLEAN in-flight by PR-8/9/10/11. Effective PR-12 delta shrunk to: (a) delete `PersistenceHelper.createPerson` + `createOasysPniResult` (0 test callers confirmed; `createPniResult` stays alive — 1 caller in `DomainEventsListenerTest.kt:222`), (b) add second offering (BXI/HMP Brixton) to `SarContractIntegrationTest` fixture for per-referral organisation variance + regen goldens, (c) optional strip trailing `\n` from `sar-api-response.json`, (d) document V145 `idx_staff_last_name` keep-vs-drop decision (recommend keep), (e) optional V144/V145 SQL comment cleanup (conditional on V146). Split out from what was originally PR-12 (docs) on 2026-08-13 pm. |
