...
- **2026-08-24 (afternoon) — Roxanne asked for a PR-14 ETA; concrete "end of week" commitment drafted.** Roxanne replied to the Thursday-sent courseName confirmation, asking whether to hold off on her testing:
  ...
  **APG-2546 close-out gating** (recap on today's message):
  ...
  | **PR-14 (courseName inline) merged** | ✅ **merged 2026-08-24 11:13 BST as [#1123](https://github.com/ministryofjustice/hmpps-accredited-programmes-api/pull/1123) → main `6d713186`** |
...
- **2026-08-24 (late afternoon) — PR-14 opened as [#1123](https://github.com/ministryofjustice/hmpps-accredited-programmes-api/pull/1123); agent-reviewed twice, awaiting team review.** Execution kicked off as committed in the ETA note above.

  **State captured at log time:**
  - Branch: `APG-2546/course-name-into-referral`
  - Commit: `ef376824` — *"APG-2546: fold courseName into referral (round-2 addendum)"*
  - PR title: *"APG-2546: fold courseName into referral (round-2 addendum)"*
  - PR state: `OPEN`, `MERGEABLE`
  - GH reviews: none posted (empty `reviews[]` on the API) — the two reviews were **internal agent nine-lens self-review passes**, not GH-side reviews. Consistent with round-2 precedent (PR-8/9/10/11/12 all self-reviewed pre-push; GH-side approval is separate).
  - CI: all checks `SUCCESS` — Kotlin `Verify the gradle app` (~7 min), CodeQL Java analyze, helm lint dev/preprod/prod, app_version, check-jira-prefix, CodeQL. Post-merge stages (`Build docker image` / `Deploy to dev` / `Deploy to preprod` / `notify_for_prod_approval` / `Deploy to prod`) `SKIPPED` as expected until merge to `main`.
  - Anchor SHA (`origin/main`): still `f84f41b2` — no new merges since Thursday; PR-14 diff applies cleanly to the SHA the doc was line-ref anchored against, so no re-anchoring risk on merge.

  **Two-review discipline followed:** matches the pattern set by PR-8 (nine-lens self-review caught the OasysPniResultEntityRepository shell inconsistency pre-push) and refined through PR-9 (two rounds of self-review before push). Both PR-14 review passes returned ship-it, no findings escalated to blocker.

  **Impact on Roxanne ETA:** on track for the *"stretch: Wednesday afternoon"* end of the commitment window sent this afternoon (best-case: team review + merge Tue am → CircleCI dev auto-deploy → HAAR single-ping for both envs at the new SHA → hold `deploy_preprod` until HAAR confirms → approve → fresh preprod PDF generable Wed pm from a rich CRN). Worst case is still Thursday/Friday within the committed "end of week" bound.

  **Impact on HAAR discipline (PR-14 doc note 7 / 2026-08-21 correction):** unchanged — on merge, PR-14 mustache bytes DO change (new `<tr>Course name</tr>` inside the referrals block, plus a re-order of the `originalReferral` sub-block's `Course name` above `Organisation name` for cross-block consistency), so the post-merge Slack ping to `#haa-sar-functionality-change-request` is a **genuine re-registration for BOTH preprod and prod at the new SHA**, not a SHA-pointer housekeeping bump. Single ping covers both envs. Hold `deploy_preprod` until HAAR confirms preprod registration; then approve.

  **APG-2546 close-out gating impact:** the "PR-14 merged" row in the gating table above flipped from *"execution to start"* to *"OPEN, agent-reviewed x2, CI green, awaiting team review"*. All other rows unchanged.


- **2026-08-24 (11:13 BST) — PR-14 merged as `#1123` → main commit `6d713186`; source branch auto-deleted.** Team review turnaround same-day; Raby squash-merged. Landed the same working day it was opened — matches the round-2 single-day cadence set by PR-8/9/10/11.

  **State captured at merge time:**
  - `origin/main` tip: **`6d713186`** — *"APG-2546: fold courseName into referral (round-2 addendum) (#1123)"*
  - Previous main tip: `f84f41b2` (unchanged since 2026-08-20 — the doc's line-ref anchor held all the way through PR-14 execution)
  - Source branch `APG-2546/course-name-into-referral`: deleted on remote (auto-delete on merge)
  - `gh pr view 1123` confirms: `state: MERGED`, `mergedAt: 2026-08-24T10:13:40Z` (11:13 BST), `mergedBy: raby`

  **Post-merge CircleCI pipeline** (auto-fires on `main` push):
  - `Build docker image` → `Deploy to dev` (auto) → `Deploy to preprod` **held pending manual approval** → `notify_for_prod_approval` → `Deploy to prod` (approval-gated)
  - **DO NOT approve `deploy_preprod` yet** — HAAR-team SAR-preprod re-registration must land first (see next bullet).

  **HAAR re-registration — action required on Raby (single ping covers both envs):**

  - **Channel:** `#haa-sar-functionality-change-request`
  - **SHA:** `6d713186`
  - **Envs:** SAR-preprod AND SAR-prod (single ping for both — corrected 2026-08-21 discipline, mustache bytes DO change with this merge so this is a genuine re-registration, not a SHA-pointer housekeeping bump)
  - **What changed byte-wise** (for the ping message): new `<tr>Course name</tr>` inside the referrals block of `sar_template.mustache`, plus the `originalReferral` sub-block's `Course name` row re-ordered above `Organisation name` for cross-block consistency
  - **Draft message shape** (adapt to Raby's voice):
    > *"Hi HAAR team — ACP SAR template updated on `main` (SHA `6d713186`, PR #1123). Mustache bytes have changed (one new field `<tr>Course name</tr>` inside the referrals block, one row re-order in the `originalReferral` sub-block for consistency). Please re-register the new template hash for both SAR-preprod and SAR-prod so the deployed image doesn't trigger a byte-mismatch alert on the next scan. I'm holding the CircleCI `deploy_preprod` approval until you confirm. Thanks."*
  - **Then:** wait for HAAR confirmation → approve `deploy_preprod` in CircleCI → verify preprod pod picks up new image → regenerate preprod PDF against a rich CRN → send to Roxanne with the "PR-14 done, here's the fresh PDF" close-out email (see `handover/roxanne-followup-course-name-reply-draft.md § post-merge close-out reply` — draft to be added when Raby is ready to send).
  - **`deploy_prod`:** approve after preprod is verified. Same HAAR ping already covered prod-side registration, so no second ping needed.

  **APG-2546 close-out gating table** — PR-14 row flipped ✅. Remaining gates:
  - Preprod + Prod SAR re-registration post-PR-14 — ⏳ awaiting HAAR confirmation on today's ping
  - Roxanne final DD sign-off — ⏳ pending fresh preprod PDF (Wed target holds; on the "stretch" end of the ETA committed this morning)

  **Roxanne ETA delivery**: on-track for the *stretch: Wednesday afternoon* end of the commitment window. If HAAR turnaround is same-afternoon, fresh preprod PDF could land Tue 2026-08-25; if HAAR is overnight, Wed 2026-08-26 am/pm. Either way inside the "end of week" bound.

  **No follow-up code work anticipated.** PR-14 was the final APG-2546 code deliverable. Everything remaining is comms + release ops.

- **2026-08-25 — Roxanne's post-preprod-PDF reply → PR-15 scoped (duplicate `<h2>Courses</h2>` block removal).** After HAAR confirmed preprod re-registration overnight Mon → Tue, Raby approved `deploy_preprod`, regenerated the preprod PDF against a rich CRN post-PR-14, and sent Roxanne the fresh PDF + close-out reply drafted 2026-08-24 (Roxanne-followup close-out reply from `handover/roxanne-followup-course-name-reply-draft.md § post-merge close-out reply`). Roxanne replied same-morning:

  > *"Hi, Thanks for this. Everything is looking good, but the only thing is as you have moved the course name into the referral, its not required at the end of the report. It is currently repeating as a Courses section at the end of the report. Are you able to remove this duplicate? Sorry for being a pain!"*

  **This is a valid, small, close-loop follow-on ask — accepted as PR-15 under APG-2546** (not spun as a round-3 ticket, mirroring the PR-14 scoping call: Roxanne's DD-review side is a distinct APG-2546 close-out gate, and this is a one-block hygiene ask directly consequential to PR-14's inline `courseName`).

  **Scope shape** (see `PR-15-remove-top-level-courses.md` for the full working doc):

  - **Same shape as PR-11 almost line for line** — remove the trailing top-level list block whose content is now covered by an inline `SarReferral` field. PR-11 did it for staff (once inline surnames landed via PR-5); PR-15 does it for courses (once inline `courseName` landed via PR-14).
  - **Files to change** (verified against `origin/main @ 6d713186` on 2026-08-25):
    - `SubjectAccessRequestService.kt` — remove `courseRepository` ctor param (L42) + import (L21) + `courses = ... .toSarCourse()` Content line (L129) + `Content.courses` field (L196) + `data class SarCourse` (L280-282) + `.toSarCourse()` ext (L437-441)
    - `CourseRepository.kt` — remove `getSarCourses` @Query method (L46-54); orphan-in-prod after this PR, following PR-11's decision on `StaffRepository.findByPrisonNumber`. Sibling methods on `CourseRepository` stay untouched (many live callers).
    - `sar_template.mustache` — remove `<h2>Courses</h2>` block at L88-98
    - `SubjectAccessRequestServiceTest.kt` — remove mock decl (L44) + ctor arg (L61) + stub (L198-202) + two assertions (L262 + L321-322) + verify (L351); `CourseEntityFactory` import (L24) **stays** (still used at L106/127/147/160 for offering seeds)
    - Snapshot goldens regen — JSON loses `"courses":[{"name":"Course 1"}],` substring; HTML loses lines 291-297. Inline `courseName` on every referral stays intact in both goldens.
  - **DD impact: NONE.** Row 34 `course.name` = `Yes` regardless — value is now sourced inline from PR-14 instead of the trailing list. **Do NOT run `dd-column-h-update.py`. Do NOT send Roxanne a fresh xlsx.** PDF-only close-out reply post-merge.
  - **HAAR re-registration: required.** Mustache bytes change (block removed) → single ping to `#haa-sar-functionality-change-request` for both SAR-preprod + SAR-prod at the new merge SHA. Hold `deploy_preprod` approval until HAAR confirms. Same discipline as PR-14 (corrected 2026-08-21 shape).
  - **PDF page count expectation:** currently 2 pages post-PR-14; likely stays at 2 after PR-15, possibly drops to 1 if the layout was near a boundary. Both are correct outcomes — flag whichever value in the merge outcome, don't panic.

  **Not in scope (Roxanne did NOT ask for these — do NOT scope-creep):**

  - The top-level `<h2>Organisation</h2>` block (mustache L160-170) — Roxanne explicitly accepted this in her 2026-08-20 email (DD row 107 canonical placement); if she symmetrically asks to remove it later, that's a separate PR-16.
  - Any `<h2>Course participation</h2>` changes — different entity (`CourseParticipationEntity`), different data source, not what Roxanne referenced. `SarCourse` vs `SarCourseParticipation` are DIFFERENT types — this is one of the two non-obvious guardrails in the working doc.

  **APG-2546 close-out gating table** — PR-14 row ✅. Remaining gates:

  - **PR-15 executed + merged** — ⏳ next code action; ready to fire fresh agent using `AGENT-PROMPT-TEMPLATE.md § PR-15`.
  - **Post-PR-15 HAAR re-registration** — ⏳ post-merge, single ping for both envs at the PR-15 merge SHA.
  - **Roxanne final DD sign-off** — ⏳ pending fresh preprod PDF after PR-15 merges + HAAR confirms + preprod redeploys. PDF-only reply (no xlsx change).
  - **`deploy_prod` approval** — ⏳ after preprod verified against PR-15 build.

  **Delivery timing:** PR-14 ETA to Roxanne was "end of week / stretch Wed pm". PR-15 is a small deletion-heavy PR (net ~−59 lines, shape identical to PR-11 which shipped same-day). Realistic timing: PR-15 opens Tue 2026-08-25 pm, agent-reviewed same-day, team-review + merge Wed 2026-08-26 am, HAAR re-registration + preprod PDF regen Wed pm, Roxanne close-out reply Wed evening / Thu am. Inside the "end of week" bound if HAAR turnaround is same-day. If HAAR is overnight, close-out slips to Thu — still inside end-of-week.

  **Round-2 story summary** (retrospective, now that the shape is fully understood): five inline-context asks landed together — PR-5 added inline POM surnames (round 1); PR-11 removed the top-level Staff list once inline covered it; PR-10 added inline `organisationName`; PR-14 added inline `courseName`; PR-15 removes the top-level Courses list once inline covers it. The one asymmetry left: top-level `<h2>Organisation</h2>` block stays (Roxanne's explicit call, 2026-08-20).

  **No further code deliverables anticipated after PR-15.** PR-15 is the final APG-2546 code deliverable.

- **2026-08-25 (planning-branch nine-lens review of PR-15 doc, pre-execution)** — full nine-lens self-review of the PR-15 planning artefacts against `origin/main @ 6d713186` before firing the execution agent. Outcome: **one miss caught + patched, zero blockers, ready for execution.**

  **Nine lenses walked:**

  1. **Correctness (line refs).** All product-code + template refs re-verified byte-exact against `6d713186`. Confirmed on-disk: `SubjectAccessRequestService.kt` L21/L42/L129/L196/L280-282/L437-441; `CourseRepository.kt` L46-54; `sar_template.mustache` L88-98; `SubjectAccessRequestServiceTest.kt` L15/L24/L44/L61/L198-202/L262/L321-322/L351. ✅
  2. **Precedent alignment (PR-11 shape).** Comparison to PR-11's doc + merge diff — same-shape confirmed: remove top-level list + orphan-in-prod @Query + ctor param + import + DTO + mapper ext + template block + test mock/assertion/verify triple. Repository interface stays alive (multiple non-SAR callers, verified via grep) — mirrors PR-11's `StaffRepository` handling exactly. ✅
  3. **Test coverage — unit + integration + snapshot.** **MISS caught:** `SubjectAccessRequestServiceIntegrationTest.kt` L147 (`assertThat(content.courses).hasSize(1)`) and L178-180 (`with(content.courses[0]) { assertThat(name).isEqualTo("Course 1") }`) were not in the initial scoping. Both would have broken the build immediately if left. Working doc + `AGENT-PROMPT-TEMPLATE.md § PR-15` + this DELIVERY-LOG scope-line updated in the same commit. `SarContractIntegrationTest.kt` remains unchanged code-side (snapshot regen only) — grep-verified no `content.courses` reference. ✅ **(after patch)**
  4. **Repository / orphan analysis.** Exhaustive grep on `SarCourse\b|getSarCourses` across the entire codebase (not just `src/`): only in-scope product-code + in-scope test-code + planning docs. Zero downstream consumers (no Pact contracts on SAR, no controller re-serialisers, no OpenAPI `@Schema` — CourseController/OrganisationController/PniController were all screened, none touch SAR content). `CourseRepository` sibling methods (`findByOrganisationId`, `findBuildingChoicesCourses`, `findAllByName`, etc.) all have live prod callers — deletion of `getSarCourses` alone does NOT trigger empty-shell restoration (contrast with PR-8's `OasysPniResultEntityRepository`). ✅
  5. **Downstream consumer impact.** `entity-schema.json` is JPA-entity-derived (not SAR-DTO-derived) — deleting `SarCourse` data class touches zero JPA entities → schema stays byte-identical (verification bullet added to checklist). `HmppsSubjectAccessRequestContent` polymorphic wrapper (kotlin-lib) doesn't care about internal `Content.courses` shape. No API surface exposed externally (SAR is a HMPPS-internal DPS integration; JSON goes to the SAR service, not client apps). ✅
  6. **HAAR / deploy discipline.** Mustache bytes change (block removed) → same corrected 2026-08-21 single-ping discipline: one Slack ping to `#haa-sar-functionality-change-request` covering both SAR-preprod AND SAR-prod at PR-15 merge SHA, hold `deploy_preprod` until HAAR confirms, then `deploy_prod` after preprod verified. Draft message shape baked into the working doc's "HAAR re-registration" section. ✅
  7. **DD impact.** Row 34 `course.name` = `Yes` regardless — value moved from top-level list to inline `SarReferral.courseName` in PR-14; PR-15 just removes the duplicated top-level render. **No `dd-column-h-update.py` run, no xlsx to Roxanne, PDF-only close-out reply.** Symmetry check with PR-11: PR-11 also had zero DD impact (Deborah's ask #5 was "add staff to referral rather than list separately" — no separate DD entity for `staff.name` at that time). Same story here. ✅
  8. **Session hygiene / execution guardrails.** Working doc's non-obvious items #1-7 cover the two hard guardrails (SarCourse ≠ SarCourseParticipation; do NOT touch Organisation block) + orphan-in-prod repo-method deletion rule + rollback semantics. AGENT-PROMPT-TEMPLATE reiterates no-zsh-heredocs rule (relevant here — planning-branch commit earlier today got bitten by this exact rule and recovered via `git commit -F`; discipline holds). Docker Desktop pre-req for Testcontainers-backed regen script also called out. ✅
  9. **Rollback + comms.** Single `git revert` restores pre-PR-15 template; pre-PR-15 template is round-2 compliant (block removal is purely subtractive-of-duplicate) → safe revert without immediate re-registration. Comms shape: Roxanne close-out reply once preprod PDF regenerates post-PR-15 merge; same PDF-only shape as the PR-14 close-out reply drafted 2026-08-24. ✅

  **Project-pattern / convention checks — separate pass, all pass:**

  - Data-class field ordering — N/A, deletion PR, no field-position choices.
  - Mustache indentation — `<h2>Courses</h2>` block sits between `<h2>Audit records</h2>` (ends L86) and `<h2>PNI results</h2>` (starts L100); deletion of L88-98 leaves adjacent blank-line pair (L87 + L99) — will collapse to single blank on regen (PR-11 precedent, "double-blank tidy" call-out).
  - Kotlin data class deletion order — mirror PR-8's ordering: remove Content-constructor line first, then Content-field, then DTO decl, then mapper ext, then ctor param + import last (import will auto-flag as unused via ktlint). PR-11 followed this order cleanly. ✅
  - `expectedFlywaySchemaVersion = "144"` — unchanged (verified: no migration in this PR).
  - Nine-lens self-review discipline for the **executing agent** stays required — same as PR-8 through PR-14. Two rounds of self-review before push, both must ship-it.
...
- **2026-08-25 (afternoon) — PR-15 opened as [#1124](https://github.com/ministryofjustice/hmpps-accredited-programmes-api/pull/1124) as DRAFT; execution agent report received.** Fresh agent fired against the nine-lens-reviewed working doc + agent prompt. Delivered exactly to spec: **7 files changed, +1 / −64 lines** (matches doc prediction of ~7 files, net ~−63). Branch `APG-2546/remove-top-level-courses` @ `b22528d1` (post-amend, see Surprise #1 below).
  ...
  **APG-2546 code work is COMPLETE.** PR-15 is the final code deliverable. Everything after this is comms + release ops.
  ...
  ~~code work COMPLETE~~ **Amended 2026-09-01** — Roxanne's follow-up email after PR-15 preprod PDF flagged one further data issue (see 2026-09-01 timeline entry). PR-16 opened to resolve. PR-15 is no longer the final code deliverable.

- **2026-08-27 15:35 UTC** — PR #1124 merged to `main` by Raby as **`10d794d9`**. Source branch `APG-2546/remove-top-level-courses` auto-deleted on merge. HAAR ping sent for both envs at `10d794d9`. HAAR re-registered SAR-preprod + SAR-prod. `deploy_preprod` approved, image picked up on preprod pod. Preprod PDF regenerated against Roxanne's rich CRN — 2 pages (unchanged from PR-14 preprod, as predicted). PDF sent to Roxanne 2026-08-27 pm via the pre-drafted close-out reply. `deploy_prod` approved 2026-08-28. Prod verified serving updated template. **PR-15 fully shipped end-to-end.**

- **2026-09-01 (morning)** — Roxanne's follow-up email after reviewing the preprod PDF she was sent 2026-08-27:
  ...
  **Diagnosis (Raby + assistant, 2026-09-01):** `LQU64X` is a NOMIS/DPS username. Root cause traced to `CourseParticipationService.buildCourseParticipation` at `L90` (mirror at `updateExistingParticipation` L68): `source = referralEntity.referrer.username`. When a course participation is auto-created from a referral, the entity's `source` column is populated with the referrer's NOMIS username code. `CourseParticipationEntity.source` is a `text` column (`V12__create_course_participation_history.sql:8`) with **two** production write paths:
  1. **Auto-derived from referral** → NOMIS username (Roxanne's `LQU64X` case).
  2. **Free-text via UI** → arbitrary user-typed string (e.g. `"OASys"`, `"self-declared"`, `"paper referral"` — verified via grep of live production records).

  The SAR view (`SubjectAccessRequestService.toSarParticipation` on `origin/main @ 10d794d9`, L209) surfaces `source` as a raw pass-through — `source = it.source` — no lookup. Every other username-typed field in the same file (8 sites, verified) resolves via `surnames.forUsername(...)` from the batch pre-fetch. `source` is the sole outlier.

  Fix approach: extend the batch pre-fetch at L110-115 to include `participation.source` (`it.source?.let(::add)` — third line in the existing `participations.forEach { ... }` block), then swap the mapper to `source = surnames.forUsername(it.source) ?: it.source`. Fallback preserves path-2 free-text values (they don't match any staff row → lookup returns null → raw preserved). This brings `source` into line with the SAR service's own convention.

- **2026-09-01 (afternoon)** — **PR-16 scoped** in `doc/planning/APG-2546/PR-16-resolve-course-participation-source.md`. All line refs pre-verified against `origin/main @ 10d794d9`:
  - `SubjectAccessRequestService.kt` L110-115 (prefetch block) + L209 (mapper) — verified via `git show`
  - `SubjectAccessRequestServiceTest.kt` L217-223 (unit assertion append target) + L48-56 (stub returns "River" for any non-blank username) — verified
  - `SubjectAccessRequestServiceIntegrationTest.kt` L109-114 (integration assertion append target) + L69 (`source = "Source"` seed) — verified
  - `SarContractIntegrationTest.kt` L150 (`source = "Source"` seed) + L216 (staff row `username = "TEST_USER"`) — verified: post-fix goldens byte-identical because `"Source"` has no matching staff row → fallback preserves the raw string
  - Goldens (`sar-api-response.json`, `sar-expected-render-result.html`, `entity-schema.json`) — expected byte-identical
  - `StaffLookupService.resolveSurnamesByUsername` (`StaffLookupService.kt:45-60`) — verified: filters nulls/blanks, tolerates non-matching inputs, returns absent keys for non-matches. Adding free-text values to input `Collection<String?>` is safe.

  **Nine-lens self-review on the PR-16 doc:** ✅ 8 lenses pass, 1 patched.
  - Lens 1 (line refs): re-verified byte-exact against `origin/main`
  - Lens 2 (precedent): mirrors existing 8 forUsername sites; **new pattern flagged** — the fallback (`forUsername(x) ?: x`) is a first-in-APG-2546. Documented explicitly under "Precedent" so the executing agent understands why it's not literal-copy of existing sites.
  - Lens 3 (test coverage): 3 assertion additions cover both paths (path 1 in unit test with mock returning "River" for "SOURCE"; path 2 in unit test with mock deliberately excluding "OASys"; path 2 in integration test with real DB and unresolvable "Source"). Contract test needs no change — golden byte-identity is the load-bearing property.
  - Lens 4 (repo / orphan): nothing becomes orphan; no repository/service changes outside the two lines in SubjectAccessRequestService.kt.
  - Lens 5 (downstream consumer impact): SAR JSON `source` type unchanged (`String?`). Only value semantics shift from "raw code" to "surname-if-known-else-raw". SAR consumer = HMPPS-SAR-proxy only (no Pact contract on SAR endpoint, confirmed earlier in APG-2546). `CourseParticipationController` CRUD API's `source` field UNTOUCHED (raw column pass-through) — write side, analytics, participation reconciliation all consume raw as today.
  - Lens 6 (HAAR / deploy discipline): mustache bytes UNCHANGED → **no HAAR ping required**, **no `deploy_preprod` hold**. This is different from PR-10/PR-11/PR-14/PR-15 which all touched the template. Doc emphasises this. Still verify preprod PDF before Roxanne close-out reply (the *value* on the PDF is what changes).
  - Lens 7 (DD impact): nil. Row for `courseParticipation.source` already `Yes` in "In SAR API"; only the display value changes. No `dd-column-h-update.py` run, PDF-only close-out reply.
  - Lens 8 (session hygiene): **PATCHED** — original doc omitted the standard reminders. Added: no zsh heredocs (use `git commit -F /tmp/apg2546-pr-16-msg.txt`), Docker Desktop required for snapshot regen, `read_file` cache staleness post-checkout, `.snyk` auto-regeneration trap (PR-15 tripped on this), `git fetch origin` SSH flakiness fallback (`git show origin/main:<path>` still works against last-fetched cache), verify anchor `10d794d9` before start.
  - Lens 9 (rollback + comms): single `git revert` restores pass-through; no template registration to reset; no cross-service coordination.

  **Additional hard guardrails** documented in doc's "Non-obvious things" section:
  - #1 fallback direction — must be `forUsername(...) ?: it.source`, not reversed (reversed is a no-op)
  - #3 do NOT touch `CourseParticipationService.kt` (write side is correct product behaviour for non-SAR consumers)
  - #5 do NOT extend `SarContract` seed to add a resolvable source (goldens byte-identity is a stronger regression-safety property than proving both paths in the golden)
  - #10 the two username-typed columns (`createdByUsername`, `lastModifiedByUsername`) are ALREADY prefetched — do not duplicate; only add `it.source?.let(::add)` as a third line
  - #11 `source` column type is `text` (unbounded) but SAR-request scale caps IN-list at ~40 entries per prisoner — no perf concern
  - #12 free-text collision with a real staff username is an accepted tradeoff (out of scope to detect); flag under "known limitations" in the PR body

  Expected delta: **4 files touched, net +~37 lines** (test-dominant); +1 product-code line added, ~1 product-code line modified; 3 goldens byte-identical after regen.

  Expected PDF page count post-fix: **2 pages, unchanged**. This PR changes one cell's value in one table row.

  **HAAR ping: NOT required** (mustache unchanged). This is the first round-2 PR to skip the HAAR gate. Deploy pipeline can proceed uninterrupted.

  Ready for execution against a fresh agent — PR-16 doc committed on planning branch. Prompt to be added to `AGENT-PROMPT-TEMPLATE.md § PR-16`.
...
| PR-14 | `PR-14-course-name-into-referral.md` | `APG-2546/course-name-into-referral` | **[#1123](https://github.com/ministryofjustice/hmpps-accredited-programmes-api/pull/1123)** | ✅ **merged 2026-08-24 11:13 BST** (opened 2026-08-24 am; agent-reviewed x2 both LGTM; CI green; team-review turnaround same-day; merged by Raby; source branch auto-deleted on merge) | **`6d713186`** | Round-2 addendum after Roxanne's 2026-08-20 DD-review ask ("would it also be possible for you to add the course name into the referral"). Natural completion of PR-10's inline-context pattern (PR-10 folded organisation into referral; PR-14 folds course name into referral). Executed against pre-verified `origin/main @ f84f41b2` anchor. Scope: `SarReferral` data class gains `courseName: String?`; `toSarReferral` mapper populates it via already-EAGER-loaded `offering.course.name` (verified on `OfferingEntity.kt:26` pre-execution; no batch pre-fetch needed, distinct from PR-10); one new `<tr>Course name</tr>` in referrals block of `sar_template.mustache`; `originalReferral` sub-block's `Course name` row re-ordered above `Organisation name` for cross-block consistency; one unit-test assertion + snapshot regen. **Two-review discipline**: two agent nine-lens self-review passes before push, both ship-it (no findings escalated to blocker). Not GH-side reviews — internal, matching PR-8/9 precedent. **Mustache bytes DO change** → post-merge HAAR ping is a genuine re-registration for both envs at the new SHA (single ping covers preprod + prod per corrected 2026-08-21 discipline). |
| PR-15 | `PR-15-remove-top-level-courses.md` | `APG-2546/remove-top-level-courses` | **[#1124](https://github.com/ministryofjustice/hmpps-accredited-programmes-api/pull/1124)** | ✅ **merged 2026-08-27 15:35 UTC** (opened 2026-08-25 as DRAFT; agent nine-lens self-reviewed x2 both ship-it; team review + merge by Raby 2026-08-27; source branch auto-deleted) | **`10d794d9`** | Second round-2 addendum. Delivered exactly to spec: **7 files, +1/−64** (matches ~7 files, net ~−63 doc prediction). Executed against `origin/main @ 6d713186` anchor. Mirror of PR-11's shape: removed `courseRepository` ctor param + import + `courses = ...` Content line + `Content.courses` field + `SarCourse` data class + `.toSarCourse()` ext from `SubjectAccessRequestService.kt`; deleted orphan-in-prod `CourseRepository.getSarCourses` @Query; deleted `<h2>Courses</h2>` block at mustache L88-98; removed mock/import/ctor-arg/stub/2 assertions/verify from `SubjectAccessRequestServiceTest.kt`; removed `content.courses` size assertion + `with(content.courses[0]) { ... }` block from `SubjectAccessRequestServiceIntegrationTest.kt` (nine-lens-caught pre-execution); goldens regenerated (JSON loses `"courses":[…]`, HTML loses lines 291-297). All five hard guardrails held. PDF page count = 2 pages, unchanged from PR-14 preprod. 678 tests green. Four surprises captured — see 2026-08-25 (afternoon) timeline entry (most notable: `.snyk` gitignore tech-debt recommendation for future hygiene PR; planning-branch stale-src drift on Flyway version + line numbers, semantic-anchor-first discipline recommended for future addendum scoping). Post-merge: HAAR re-registered both envs at `10d794d9`; preprod PDF sent to Roxanne 2026-08-27 pm; prod deployed 2026-08-28. |
| PR-16 | `PR-16-resolve-course-participation-source.md` | `APG-2546/resolve-course-participation-source` | _(pending — not yet opened)_ | ⏳ **scoped 2026-09-01 pm** (planning-branch doc committed; nine-lens self-review complete — 8 pass, 1 patched; anchors verified against `origin/main @ 10d794d9`; ready for execution agent) | pending | Third round-2 addendum. Triggered by Roxanne's 2026-09-01 email on the post-PR-15 preprod PDF flagging `source = LQU64X` under Course participation. Root cause: `CourseParticipationEntity.source` is dual-purpose — auto-derived from `referral.referrer.username` (NOMIS username, path 1) or free-text via UI (path 2, values like `"OASys"`). Every other username-typed SAR field is resolved via `surnames.forUsername(...)` (8 sites verified on `10d794d9`); `source` is the sole raw pass-through. Fix: 2-line change — extend batch prefetch to include `it.source?.let(::add)` in `resolveStaffSurnames` participation block (L110-115), swap mapper to `source = surnames.forUsername(it.source) ?: it.source` at L209. Fallback direction is load-bearing (path-2 preservation). **First round-2 PR to skip HAAR ping** — mustache bytes UNCHANGED (data-only fix). No `deploy_preprod` hold. Goldens byte-identical post-regen (contract test seed `source = "Source"` has no matching staff → fallback preserves). Test coverage: 3 assertion additions (1 unit for path 1 → "River"; 1 new unit test for path 2 fallback; 1 integration for path 2 real DB). Expected delta: 4 files, +~37/−2 (test-dominant), 1 product-code line added, ~1 modified. Expected PDF page count post-fix: 2, unchanged. DD impact: nil. Rollback: single `git revert`. Nine-lens review flagged and patched the missing session-hygiene section (no zsh heredocs, Docker Desktop for regen, `.snyk` trap, `read_file` cache staleness, `git fetch` SSH flakiness fallback). Twelve hard guardrails documented; three are novel to PR-16 (fallback direction, source column being `text` unbounded, free-text/username collision as accepted tradeoff). |
