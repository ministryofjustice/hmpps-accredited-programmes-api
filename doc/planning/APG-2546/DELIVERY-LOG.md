...
- **2026-08-14** — PR-8 opened as **#1116** on branch `APG-2546/remove-pni-oasys-person` (head `1b986744`, base `origin/main @ 0cf89850`). Clean execution against the doc; fresh-checkout warning caught the planning-branch worktree drift (agent switched to `origin/main` before reading files, exactly as R6 intended). Two within-scope orphan-cleanup extensions of doc directives — both R3-spirit, both called out on the outcomes table row above: whole-file deletion of `OasysPniResultEntityRepository.kt` (only method was the SAR call site) and removal of three now-dead SAR-service ctor params (verified single public entry point + no other in-class callers). Verification: 8 files (+1 / -365), 678 tests pass, ktlint clean, 0 UUID leaks, 0 PACT contracts on SAR endpoint, no `@Operation`/`@Schema` on SAR controller (OpenAPI check N/A confirmed — matches doc prediction), `entity-schema.json` unchanged (JPA entities kept alive by non-SAR callers). Test-harness (Option 2) PDF still 3 pages — removed sections did not cross a page boundary in the chrome-less render. PR-9 unblocked to start from a fresh session once #1116 merges.
...
|---|---|---|---|---|---|---|
| PR-8 | `PR-8-remove-pni-oasys-person.md` | `APG-2546/remove-pni-oasys-person` | **#1116** | open, awaiting review (head `1b986744`) | _pending_ | Doc followed literally. Two within-scope orphan-cleanup extensions of doc directives (R3 spirit), both called out on the PR body: (a) deleted whole `OasysPniResultEntityRepository.kt` file — its only `@Query` was the SAR call site, empty interface would have been picked up by PR-12 hygiene anyway; (b) removed three SAR-service ctor params (`pniResultRepository`, `personRepository`, `oasysPniResultEntityRepository`) after verifying only one public entry point in the class and each repo used only at removed call sites; `PersonRepository` interface stays alive (6 other prod callers), only its SAR ctor injection removed. 8 files, +1 / -365. 678 tests pass, ktlint clean, 0 UUID leaks, 0 PACT contracts on SAR endpoint, no `@Operation`/`@Schema` on SAR controller (OpenAPI N/A confirmed), `entity-schema.json` unchanged. Test-harness (Option 2) PDF = 3 pages, unchanged from round-1 baseline (removed sections did not cross a page boundary in the chrome-less artefact); Option 1 (Branston-facing) PDF is PR-13's job. |
