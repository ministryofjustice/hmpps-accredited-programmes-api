...
| PR-15 | `PR-15-remove-top-level-courses.md` | `APG-2546/remove-top-level-courses` | _pending_ | ⏳ **ready for execution 2026-08-25** — scoped after Roxanne's 2026-08-25 reply to the preprod PDF ("as you have moved the course name into the referral, its not required at the end of the report … Are you able to remove this duplicate?"). Line refs verified against `origin/main @ 6d713186`. | _pending_ | Second round-2 addendum. Mirrors PR-11's shape almost line for line — remove trailing top-level list block once inline `SarReferral` field covers the content (PR-11 did staff post-PR-5 inline surnames; PR-15 does courses post-PR-14 inline `courseName`). Scope: remove `courseRepository` ctor param + import + `courses = ...` Content line + `Content.courses` field + `SarCourse` data class + `.toSarCourse()` ext from `SubjectAccessRequestService.kt`; remove orphan-in-prod `CourseRepository.getSarCourses` @Query (PR-11 precedent); remove `<h2>Courses</h2>` block at mustache L88-98; remove mock/import/ctor-arg/stub/assertions/verify from `SubjectAccessRequestServiceTest.kt`; regen goldens (JSON loses `"courses":[…]` substring, HTML loses lines 291-297). Net ~−59 lines. **DD impact: NONE** (row 34 `course.name` = Yes regardless, sourced inline post-PR-14). **HAAR ping required post-merge** (mustache bytes change, same single-ping discipline as PR-14). Not in scope: top-level `<h2>Organisation</h2>` block (Roxanne accepted 2026-08-20), and any `<h2>Course participation</h2>` changes (different entity — hard guardrail #1 in working doc). |
