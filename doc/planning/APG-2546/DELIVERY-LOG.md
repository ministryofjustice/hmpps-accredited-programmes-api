...
| PR-13 | `PR-13-round-2-docs-and-handover.md` | `APG-2546/round-2-docs-handover` | _pending_ | ⬜ **ready to start 2026-08-18** — anchored to `99264496`, doc expanded to fully agent-executable | _pending_ | _pending_ | **Unblocked by PR-12 merge.** Working doc rewritten 2026-08-18 into fully agent-executable shape (see 2026-08-18 timeline entry): full ground-truth verification section against `99264496`, prerequisites, execution-order checklist, three-track scope (DELIVERY-LOG close-out / R7 `.gitignore` sweep / Branston-facing handover PDF), all four email/Slack templates drafted, verification checklist, PR body template, DoD. Four scope items already folded in from PR-12 self-review still apply verbatim: (a) call out per-referral organisation variance in Branston-facing PDF walkthrough — fixture now renders HMP Brixton × 2 + HMP Moorland × 1, worth a "here's how per-referral wiring looks under two-organisation load" beat; (b) reference the V145 keep-with-reasoning decision by linking PR #1120 rather than re-litigating; (c) refresh helper-method inventory if the handover PDF enumerates (unlikely — flag only); (d) add `.gitignore` / `.git/info/exclude` sweep for `.snyk` + `Copy of *.xlsx` under `doc/` as one-line R7 close-out. |
