...
| R3 | Repository queries removed in PR-8 / PR-11 turn out to have other callers. | Each PR: `grep -r <queryName>` across `src/main` before deletion. If any non-SAR caller hit, leave the query alive (cheap) and just remove the SAR-service call site. **Learning added 2026-08-17 from PR-10 + PR-11**: when removing a top-level `Content` collection, the matching repository **constructor param + import** in `SubjectAccessRequestService.kt` become dead code that ktlint's no-unused-imports rule will catch on the import but will silently linger on the ctor param. Sweep both in the same PR (and mirror on the unit test's `setup()` — drop the `mockk()` decl + ctor arg). Precedent: PR-10 (`organisationRepository` — kept, still had `SarOriginalReferral` consumer), PR-11 (`staffRepository` — removed). |
...
| R7 | `.snyk` + `Copy of *.xlsx` untracked files reappear in `git status` on every planning session and risk being accidentally staged. | ✅ **Accepted paper-cut / deferred out of APG-2546 (2026-08-18).** Considered a two-line `.gitignore`-only PR to `main`; rejected as scope-creep — coupling APG-2546 close-out to another review cycle for two ignore-pattern lines is a bad trade, and every round-2 PR agent (PR-8/9/10/11/12) discipline-gated on `git status --short` before commit without incident. Fold a two-line fix into the next adjacent code PR on some other ticket if the noise re-annoys us. Interim gate (`git status --short` before commit) stays in every PR checklist. |
