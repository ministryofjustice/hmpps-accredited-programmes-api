...
-- list in a natural read order. StaffRepository.findByPrisonNumber
-- uses `ORDER BY s.lastName, s.staffId` to guarantee determinism (and
-- to render a sensible order on the PDF); this index backs the sort
-- so we're not scanning the full staff table for every SAR request.
--
