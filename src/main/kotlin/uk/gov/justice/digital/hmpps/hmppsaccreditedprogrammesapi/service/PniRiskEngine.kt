...
class PniRiskEngine {
  ...
  fun isHighIntensityBasedOnRiskScores(individualRiskScores: IndividualRiskScores) = (isHighOgrs3(individualRiskScores) && (isHighOvp(individualRiskScores) || isHighSara(individualRiskScores)))
  ...
  private fun isHighOgrs3(individualRiskScores: IndividualRiskScores) = individualRiskScores.ogrs3?.let { it >= BigDecimal("75.00") } == true

  private fun isHighOvp(individualRiskScores: IndividualRiskScores) = individualRiskScores.ovp?.let { it >= BigDecimal("60.00") } == true

  private fun isOspDcHigh(individualRiskScores: IndividualRiskScores, gender: String): Boolean = gender.equals("Male", ignoreCase = true) &&
    (individualRiskScores.ospDc?.contains(RiskClassification.HIGH_RISK.description, ignoreCase = true) == true)

  private fun isOspIicHigh(individualRiskScores: IndividualRiskScores, gender: String): Boolean = gender.equals("Male", ignoreCase = true) &&
    (individualRiskScores.ospIic?.contains(RiskClassification.HIGH_RISK.description, ignoreCase = true) == true)

  fun isHighSara(individualRiskScores: IndividualRiskScores) = (individualRiskScores.sara?.saraRiskOfViolenceTowardsOthers?.contains(RiskClassification.HIGH_RISK.description, ignoreCase = true) == true) ||
  ...
  private fun isOgrs3Medium(individualRiskScores: IndividualRiskScores) = individualRiskScores.ogrs3?.let { it in BigDecimal("50.00")..BigDecimal("74.00") } == true

  private fun isOvpMedium(individualRiskScores: IndividualRiskScores) = individualRiskScores.ovp?.let { it in BigDecimal("30.00")..BigDecimal("59.00") } == true

  private fun isOspDcMedium(individualRiskScores: IndividualRiskScores, gender: String): Boolean = gender.equals("Male", ignoreCase = true) &&
    individualRiskScores.ospDc?.equals(RiskClassification.MEDIUM_RISK.description, ignoreCase = true) == true

  private fun isOspIicMedium(individualRiskScores: IndividualRiskScores, gender: String): Boolean = gender.equals("Male", ignoreCase = true) &&
    individualRiskScores.ospIic?.equals(RiskClassification.MEDIUM_RISK.description, ignoreCase = true) == true

  fun isMediumSara(individualRiskScores: IndividualRiskScores) = individualRiskScores.sara?.saraRiskOfViolenceTowardsOthers?.equals(RiskClassification.MEDIUM_RISK.description, ignoreCase = true) == true ||
