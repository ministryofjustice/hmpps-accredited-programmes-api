...
class ApiExceptionHandlerTest {
  ...
  fun `prisonApiUnavailableHandledAsExpected 503 error response with appropriate message`() {
    val exception = PrisonApiUnavailableException("Prison API is unavailable", Throwable("Service is down"))
    val response = handler.handleServiceUnavailableException(exception)

    assertThat(response.statusCode).isEqualTo(HttpStatus.INTERNAL_SERVER_ERROR)
    assertThat(response.body?.userMessage).isEqualTo("Service unavailable: Prison api is not available at the moment, please try after sometime")
